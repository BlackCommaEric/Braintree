...
$result = Braintree\Transaction::sale([ //take resulting sale data from BT servers and create a variable containing all of the object's data. Call each data point with e.g. $result->transaction->amount, etc.
    ...
    'options' => [
        'submitForSettlement' => true //sets whethert he transaction is now final or has been processed to settle
    ]
]);
...
if ($result->success || !is_null($result->transaction)) { //if the transaction shows as a success (success = 1) OR if the transaction results are not empty, do the following:
    ...
    header("Location: transaction.php?id=" . $transaction->id); //send the user to the transaction_hosted.php page with the trsnaction id appended to the URL
    ...
    header("Location: index.php"); //append error string above to url and send back to beginning of transaction flow to start over
}
