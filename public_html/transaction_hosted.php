...
<?php
    ...
    if (isset($_GET["id"])) { //if a transaction id is present in the URL, do the following:
        $transaction = Braintree\Transaction::find($_GET["id"]); //retrieve the transaction by its id and set it as the $transaction variable

        $transactionSuccessStatuses = [ //sets the various transaction statuses available for a success
        ...
        if (in_array($transaction->status, $transactionSuccessStatuses)) { //if the transaction's status ($transaction->status) matches one of the $transactionSuccessStatuses above, use the following information for the page's HTML:
            ...
            $icon = "fail";
            $message = "Your test transaction has a status of " . $transaction->status . ". See the Braintree API response and try again."; //display the transaction's status within the page's HTML
        }
    }
?>
...
<div class="wrapper">
    <div class="response container">
        <div class="content">
            ...
            <section>
                <a class="button primary back" href="hosted-fields.php">
